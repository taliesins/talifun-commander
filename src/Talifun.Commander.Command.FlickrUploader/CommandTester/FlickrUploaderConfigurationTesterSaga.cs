...
using Talifun.Commander.Command.FlickrUploader.CommandTester.Response;
using Talifun.Commander.Command.FlickrUploader.Configuration;

namespace Talifun.Commander.Command.FlickrUploader.CommandTester
{
	[Serializable]
	public class FlickrUploaderConfigurationTesterSaga : SagaStateMachine<FlickrUploaderConfigurationTesterSaga>, ISaga
	{
		static FlickrUploaderConfigurationTesterSaga()
		{
			Define(() =>
			{
				Initially(
		...
		public static Event<FlickrUploaderConfigurationTestRequestMessage> TestConfigurationRequestEvent { get; set; }

		public FlickrUploaderConfigurationTesterSaga(Guid correlationId)
		{
		...
		private ISettingConfiguration Settings
		{
			get
			{
				return FlickrUploaderConfiguration.Instance;
			}
		}

		private void Consume(FlickrUploaderConfigurationTestRequestMessage message)
		{
			var responseMessage = new FlickrUploaderConfigurationTestResponseMessage()
			{
			...
			Bus.MessageContext<FlickrUploaderConfigurationTestRequestMessage>().Respond(responseMessage); 
		}
	}
}
